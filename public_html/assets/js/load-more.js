...
const paging = {
  more: function () {
    // Show the loader
    paging.showLoader(); // Disable the button

    paging.disableButton(); // Grab and set the next-url

    var next = paging.next(); // If the next-url is defined, load more elements
    ...
    if (next) {
      fetch(next, {
        ...
        loadMoreWrapper.innerHTML += data; // Enable the button

        paging.enableButton(); // No more loading

        paging.hideLoader(); // Hide the button if the end is reached
  ...
  hideButtonOnEnd: function () {
    ...
    let next = paging.next(); // Hide the button
