...
rsync -azhcvv -e "ssh $SSH_OPTS" --delete --exclude '.gitkeep' --exclude 'manifests/' --exclude 'fonts/' dist/assets dist/public_html/manifest.json dist/public_html/sw.js dist/public_html/robots_staging.txt  $SSH_HOST:./$DEPLOY_DIR/public_html/
...
# Set robots.txt for staging
ssh $SSH_OPTS $SSH_HOST "cd $DEPLOY_DIR && cd public_html/ && mv robots_staging.txt robots.txt"
# check media
...
ssh $SSH_OPTS $SSH_HOST "export PATH=/usr/local/php74/bin:$$PATH; cd $DEPLOY_DIR; ./craft backup --zip --overwrite && ./craft migrate/all --interactive 0 && ./craft project-config/apply --interactive 0 && ./craft clear-caches/all && ./craft seomatic/sitemap/generate"
